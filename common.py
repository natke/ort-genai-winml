...
def register_ep(ep: str, ep_path: str, use_winml: bool, providers: list[str] | None = None) -> None:
    ...
    if use_winml:
        ...
        try:
            ...
            print("Failed to import winml.py. Ensure winml.py is available in the current directory.")
            return
        ...
        try:
            ...
            print("Ensure 'winrt-runtime', 'winui3', and 'onnxruntime_genai' packages are installed.")
            return
        except ModuleNotFoundError as e:
            # winui3 submodules not found (e.g. winui3.microsoft.windows.ai.machinelearning)
            print(f"Missing WinML module: {e}")
            print("Ensure the 'winui3' package is installed with Windows ML support.")
            return
        except AttributeError as e:
            # WinML API not available on this version of Windows
            print(f"WinML API not supported on this version of Windows: {e}")
            print("Ensure you are running a compatible version of Windows with the required WinML APIs.")
            return
        except OSError as e:
            # Windows App SDK runtime not installed or provider library not found on disk
            print(f"OS error during WinML EP registration (Windows App SDK may not be installed): {e}")
            print("Install the Windows App SDK runtime: https://learn.microsoft.com/en-us/windows/apps/windows-app-sdk/downloads")
            return
        except RuntimeError as e:
            # EP registration failures from ORT GenAI (e.g. incompatible library, provider not ready)
            print(f"Runtime error registering WinML execution providers: {e}")
            print("A provider library may be incompatible or a driver may be missing.")
            return
        except Exception as e:
            # Catch-all for unexpected errors
            print(f"Unexpected error registering WinML execution providers: {type(e).__name__}: {e}")
            return
    elif ep == "cuda":
